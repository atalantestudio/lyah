...
namespace vec2_m128 {
	...
	void testVectorMatrixMultiplication() {
		...
		const lyah::vec<2, std::float_t> x = {1.0f, 4.0f};

		lyah::vec<2, std::float_t> result = x * A;
	...
	void testLerpVectorInterpolator() {
		...
		const lyah::vec<2, std::float_t> a = {1.0f, 4.0f};
		const lyah::vec<2, std::float_t> b = {5.0f, 3.0f};
		const lyah::vec<2, std::float_t> t(0.3f);
	...
	void testDegrees() {
		const lyah::vec<2, std::float_t> expected = {180.0f, -45.0f};
		const lyah::vec<2, std::float_t> a = {lyah::pi<std::float_t>(), -lyah::pi<std::float_t>() * 0.25f};

		const lyah::vec<2, std::float_t> result = lyah::degrees(a);

		test::assert(test::eq(result, expected, 0.001f));
	}

	void testRadians() {
		const lyah::vec<2, std::float_t> expected = {lyah::pi<std::float_t>(), -lyah::pi<std::float_t>() * 0.25f};
		const lyah::vec<2, std::float_t> a = {180.0f, -45.0f};

		const lyah::vec<2, std::float_t> result = lyah::radians(a);

		test::assert(test::eq(result, expected, 0.001f));
	}

	void testSin() {
		const lyah::vec<2, std::float_t> expected = {0.841f, 0.479f};
		const lyah::vec<2, std::float_t> a = {1.0f, 0.5f};

		const lyah::vec<2, std::float_t> result = lyah::sin(a);

		test::assert(test::eq(result, expected, 0.001f));
	}

	void testCos() {
		const lyah::vec<2, std::float_t> expected = {0.540f, 0.878f};
		const lyah::vec<2, std::float_t> a = {1.0f, 0.5f};

		const lyah::vec<2, std::float_t> result = lyah::cos(a);

		test::assert(test::eq(result, expected, 0.001f));
	}

	void testTan() {
		const lyah::vec<2, std::float_t> expected = {1.557f, 0.546f};
		const lyah::vec<2, std::float_t> a = {1.0f, 0.5f};

		const lyah::vec<2, std::float_t> result = lyah::tan(a);
