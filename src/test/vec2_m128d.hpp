...
namespace vec2_m128d {
	...
	void testRcp() {
		const lyah::vec<2, std::double_t> expected = {lyah::infinity<std::double_t>(), 0.25};
		const lyah::vec<2, std::double_t> a = {0.0, 4.0};

		const lyah::vec<2, std::double_t> result = lyah::rcp(a);
	...
	void testRsqrt() {
		const lyah::vec<2, std::double_t> expected = {1.0, 0.5};
		const lyah::vec<2, std::double_t> a = {1.0, 4.0};

		const lyah::vec<2, std::double_t> result = lyah::rsqrt(a);
